...
dt[L0LC2443_new>=150 , TRIGLY:=1]

# HDL
dt[L0VC2420_new<40 & SEXTYP == "M", HDL:=1]
dt[L0VC2420_new>=40 & SEXTYP == "M", HDL:=0]

dt[L0VC2420_new<50 & SEXTYP == "F", HDL:=1]
dt[L0VC2420_new>=50 & SEXTYP == "F", HDL:=0]

# 혈압
dt[, BP:=ifelse(HB001_new >= 130 | HB002_new >= 85 | Hypertensiontrt == 1, 1, 0)]

# 혈당
dt[,`BloodGlucose` := ifelse(L0MC3711_new>=100 | DMtrt == 1, 1, 0)]


dt[, sum:= WAIST + TRIGLY + HDL + BP + BloodGlucose]


dt[,MetS := ifelse(sum>=3, 1, 0)]
dt[,table(MetS, useNA = "always")]


# 첫 시점에 발생한 사람 제거
dt[,id:=rowid(PATNUM)]
firstocc <- dt[id==1 & MetS==1, PATNUM]
...
a = dttmp[,.(PATNUM, ORDDAT)]
#날짜별로 봤을 때 중복되는게 있나? 78건 39명 특수일반 같이 받으신분
# 특수검진으로 고른다.
b = dttmp[duplicated(a)|duplicated(a, fromLast = T),]
...
dttmp[,cum1:=cumsum(MetS), by= PATNUM]
...
dt[, gr:=as.numeric(sum(MetS)>0),by = PATNUM]
...
dt[filter==1 & gr==1, MetS:=1]

# mean imputation
dt[MetS==1, stop:=round((start+stop)/2)]

# 변수순서정리 및 삭제
setcolorder(dt, c("PATNUM", "start", "stop", "Tend", "MetS", "gr",
...
dt[E_new>=2 & E_new < 4, Exercise:="1"]
dt[E_new>=4, Exercise:="2"]
table(dt$Exercise)


fwrite(dt, "MetSforsurv.csv")
